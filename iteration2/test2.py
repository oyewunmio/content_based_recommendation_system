...
# importing packages for the recommender system
...
class Recommender ( tk.Tk ):
    ...
    def main(self):
        ...
        self.id1 = tk.Entry(Frame, width=26)
        self.id1.place(relx=0.15, rely=0.05)

        self.id2 = tk.Entry(Frame, width=26, validate = 'key' , validatecommand = self.vcmd)
        ...
        self.N = tk.Spinbox ( Frame , from_ = 1 , to = 100 , width = 15, state='normal')
        self.N.place(relx= 0.61, rely =0.05)
        

        # creating the calculate button
        button = tk.Button(Frame, text='Calculate Similarity', width=16, command = lambda: [f () for f in [self.calculate,self.validator2]])
        button.place(relx = 0.50, rely=0.30)

        # creating the evalution button
        button = tk.Button(Frame, text='Evaluation', width=10)
        button.place(relx = 0.80, rely=0.30)
    ...
    def calculate(self):
        ...
        if self.sim_choosen.get() == 1:
            mus_sim=sim.music_similiar(data.Music_features(), self.id1.get(), self.id2.get(), self.metric.get(), 'True')
            self.Score.configure(text=str(mus_sim))
    ...
    def Time_to_run(self):
        'calculates the total time the code takes to run'
